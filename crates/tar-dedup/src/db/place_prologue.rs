...
pub fn set_placement_prologue_done(conn: &Connection) -> crate::error::Result<()> {
    meta::set_placement_prologue_done(conn, true)
}

pub fn out_tree_is_built(conn: &Connection) -> crate::error::Result<bool> {
    Ok(meta::get_out_tree_built(conn)?.unwrap_or(false))
}

pub fn set_out_tree_built(conn: &Connection) -> crate::error::Result<()> {
...
pub fn set_file_new_name(conn: &Connection, file_id: FileId, new_name: Option<&str>) -> crate::error::Result<()> {
    conn.execute(
...
pub fn insert_out_tree_rows(conn: &Connection, rows: &[NewOutTreeRow]) -> crate::error::Result<Vec<OutTreeId>> {
    if rows.is_empty() {
        return Ok(Vec::new());
    }
