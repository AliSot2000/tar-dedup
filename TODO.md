...
|          | `--ignore-failed-read`           | TO_IMPLEMENT | do not exit with nonzero on unreadable files                      | Related to fail-fast / error policy in Reflection.                                                                  | / archive          |
...
|          | `--hard-dereference`                                | TO_IMPLEMENT | follow hard links; archive and dump the files they refer to | Dedup already collapses identical content.                                                                                                        | inventory / archive |
| `-h`     | `--dereference`                                     | TO_IMPLEMENT | follow symlinks; archive and dump the files they point to   | Stage uses symlinks then tar follows (`follow_symlinks`); inventory stores link targets. GNU `-h` semantics may differ.                           | inventory / archive |
| `-K`     | `--starting-file=MEMBER-NAME`                       | DISCARDED    | begin at member MEMBER-NAME when reading the archive        |                                                                                                                                                   | -                   |
|          | `--newer-mtime=DATE`                                | TO_IMPLEMENT | compare date and time when data changed only                |                                                                                                                                                   | filter / archive    |
| `-N`     | `--newer=DATE-OR-FILE`, `--after-date=DATE-OR-FILE` | TO_IMPLEMENT | only store files newer than DATE-OR-FILE                    |                                                                                                                                                   | filter / archive    |
|          | `--one-file-system`                                 | TO_IMPLEMENT | stay in local file system when creating archive             |                                                                                                                                                   | inventory / archive |
| `-P`     | `--absolute-names`                                  | TO_IMPLEMENT | don't strip leading `/`s from file names                    | Rel-paths from input root today. (Store original -i directory in the meta table) - alternatively use abs path always. Useful if --files-from=FILE | inventory / archive |
...
|          | `--exclude=PATTERN`               | TO_IMPLEMENT | exclude files, given as a PATTERN                                                       | TODO.md Filter / Exclude.                                                            | filter / archive    |
...
|          | `--no-null`                       | TO_IMPLEMENT | disable the effect of the previous `--null` option                                      | With `--files-from`.                                                                 | filter / archive    |
|          | `--no-recursion`                  | TO_IMPLEMENT | avoid descending automatically in directories                                           | Default today is recurse full tree.                                                  | inventory / archive |
...
|          | `--null`                          | TO_IMPLEMENT | `-T` reads null-terminated names; implies `--verbatim-files-from`                       |                                                                                      | filter / archive    |
...
| `-T`     | `--files-from=FILE`               | TO_IMPLEMENT | get names to extract or create from FILE                                                |                                                                                      | inventory / archive |
...
| shortopt | longopt                      | implement    | description                                          | comment                         | phase[s] / command |
|----------|------------------------------|--------------|------------------------------------------------------|---------------------------------|--------------------|
|          | `--anchored`                 | TO_IMPLEMENT | patterns match file name start                       | Meaningful once exclude exists. | filter / archive   |
|          | `--ignore-case`              | TO_IMPLEMENT | ignore case                                          |                                 | filter / archive   |
|          | `--no-anchored`              | TO_IMPLEMENT | patterns match after any `/` (default for exclusion) |                                 | filter / archive   |
|          | `--no-ignore-case`           | TO_IMPLEMENT | case sensitive matching (default)                    |                                 | filter / archive   |
