...
|          | `--owner-map=FILE`                             | IMPLEMENTED | use FILE to map file owner UIDs and names                                                             |                                                                                           | permissions / extract |
|          | `--numeric-owner`                              | IMPLEMENTED | always use numbers for user/group names                                                               | DB stores numeric uid/gid already.                                                        | permissions / extract |
|          | `--delay-directory-restore`                    | DISCARDED   | delay setting modification times and permissions of extracted directories until the end of extraction | Matches bottom-up permissions plan in TODO.                                               | permissions / extract |
|          | `--no-delay-directory-restore`                 | DISCARDED   | cancel the effect of `--delay-directory-restore`                                                      | ??? How --no-delay-directory-restore --delay-directory-restore                            | permissions / extract |
|          | `--same-owner`                                 | IMPLEMENTED | try extracting files with the same ownership as exists in the archive                                 | CLI `--restore-owner` exists; full apply still TODO.                                      | permissions / extract |
|          | `--no-same-owner`                              | IMPLEMENTED | extract files as yourself (default for ordinary users)                                                | Default when `--restore-owner` omitted. (call query user, group of running process)       | permissions / extract |
| `-p`     | `--preserve-permissions`, `--same-permissions` | IMPLEMENTED | extract information about file permissions                                                            | Capture on archive done; apply on extract TODO.                                           | permissions / extract |
|          | `--no-same-permissions`                        | IMPLEMENTED | apply the user's umask when extracting permissions from the archive                                   | Permissions stage still stub.                                                             | permissions / extract |
|          | `--mode=CHANGES`                               | IMPLEMENTED | force (symbolic) mode CHANGES for added files                                                         | ??? What is this ???                                                                      | permissions / extract |
...
| shortopt | longopt                    | implement   | description                                                           | comment                                                                                                                                | phase[s] / command    |
|----------|----------------------------|-------------|-----------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------|-----------------------|
|          | `--keep-directory-symlink` | IMPLEMENTED | preserve existing symlinks to directories when extracting             | if path is symlink `name -> something`. defualt `rm name && mkdir name` flag changes to follow symlink instead.                        | place / extract       |
|          | `--keep-newer-files`       | IMPLEMENTED | don't replace existing files that are newer than their archive copies | Aligns with extract mtime-only update notes.                                                                                           | place / extract       |
| `-k`     | `--keep-old-files`         | IMPLEMENTED | don't replace existing files when extracting, treat them as errors    |                                                                                                                                        | place / extract       |
|          | `--skip-old-files`         | IMPLEMENTED | don't replace existing files when extracting, silently skip over them |                                                                                                                                        | place / extract       |
|          | `--overwrite`              | IMPLEMENTED | overwrite existing files when extracting                              | Place currently copies; policy flags not wired.                                                                                        | place / extract       |
|          | `--overwrite-dir`          | IMPLEMENTED | overwrite metadata of existing directories when extracting (default)  |                                                                                                                                        | permissions / extract |
|          | `--no-overwrite-dir`       | DISCARDED   | preserve metadata of existing directories                             |                                                                                                                                        | permissions / extract |
|          | `--one-top-level[=DIR]`    | IMPLEMENTED | create a subdirectory to avoid having loose files extracted           | In tar, if a loose file without dir starts archive, create tarname as defualt or DIR if arg provided. Related to master/ layout ideas. | place / extract       |
|          | `--recursive-unlink`       | DISCARDED   | empty hierarchies prior to extracting directory                       | Is used to call clear newly created directory in tar process. Currently maybe a feature but nothing more.                              | place / extract       |
|          | `--remove-files`           | DISCARDED   | remove files after adding them to the archive                         | Dangerous; not a tar-dedup goal.                                                                                                       | -                     |
| `-U`     | `--unlink-first`           | IMPLEMENTED | remove each file prior to extracting over it                          | call rm on file first and wirte into fresh file. (prevent hardlink issues)                                                             | place / extract       |
...
| shortopt | longopt                                        | implement   | description                                                   | comment                   | phase[s] / command |
|----------|------------------------------------------------|-------------|---------------------------------------------------------------|---------------------------|--------------------|
|          | `--strip-components=NUMBER`                    | IMPLEMENTED | strip NUMBER leading components from file names on extraction | Requires new field in db! | place / extract    |
|          | `--transform=EXPRESSION`, `--xform=EXPRESSION` | IMPLEMENTED | use sed replace EXPRESSION to transform file names            | Requires new field in db! | place / extract    |
