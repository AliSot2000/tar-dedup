...
|          | `--hole-detection=TYPE`          | IMPLEMENTED, FEATURE | technique to detect holes                                         | sparse version implemented. raw (basically what sparse pass does just writes sparse tar header, feature for future) |
...
| shortopt | longopt                           | implement   | description                                                                             | comment                                                                              |
|----------|-----------------------------------|-------------|-----------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------|
|          | `--add-file=FILE`                 | FEATURE     | add given FILE to the archive (useful if its name starts with a dash)                   | Paths come from `-i` tree walk today.                                                |
...
|          | `--no-unquote`                    | FEATURE     | do not unquote input file or member names                                               |                                                                                      |
...
|          | `--unquote`                       | FEATURE     | unquote input file or member names (default)                                            |                                                                                      |
...
| shortopt | longopt                    | implement    | description                                                           | comment                                                                                                                                |
|----------|----------------------------|--------------|-----------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------|
|          | `--keep-directory-symlink` | FEATURE      | preserve existing symlinks to directories when extracting             | if path is symlink `name -> something`. defualt `rm name && mkdir name` flag changes to follow symlink instead.                        |
...
|          | `--no-overwrite-dir`       | FEATURE      | preserve metadata of existing directories                             |                                                                                                                                        |
|          | `--one-top-level[=DIR]`    | FEATURE      | create a subdirectory to avoid having loose files extracted           | In tar, if a loose file without dir starts archive, create tarname as defualt or DIR if arg provided. Related to master/ layout ideas. |
...
|          | `--recursive-unlink`       | FEATURE      | empty hierarchies prior to extracting directory                       | If we extract /foo/bar/baz and baz already exists, with this option rm -rf /foo/bar/baz/* is called and files extracted into later.    |
...
| `-U`     | `--unlink-first`           | FEATURE      | remove each file prior to extracting over it                          | call rm on file first and wirte into fresh file. (prevent hardlink issues)                                                             |
...
| shortopt | longopt                                        | implement    | description                                                                                           | comment                                                                           |
|----------|------------------------------------------------|--------------|-------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------|
|          | `--atime-preserve[=METHOD]`                    | TO_IMPLEMENT | preserve access times on dumped files                                                                 | Times captured in inventory; restore policy unclear.                              |
|          | `--clamp-mtime`                                | FEATURE      | only set time when the file is more recent than what was given with `--mtime`                         | No `--mtime` force path yet.                                                      |
...
|          | `--no-same-permissions`                        | TO_IMPLEMENT | apply the user's umask when extracting permissions from the archive                                   | Permissions stage still stub.                                                     |
|          | `--numeric-owner`                              | TO_IMPLEMENT | always use numbers for user/group names                                                               | DB stores numeric uid/gid already.                                                |
...
|          | `--sort=ORDER`                                 | FEATURE      | directory sorting order: none (default), name or inode                                                | Staging orders by ext/size/id for compression. Feature scan order. (ORDER BY key) |
...
| `-h`     | `--dereference`                                     | FEATURE    | follow symlinks; archive and dump the files they point to   | Stage uses symlinks then tar follows (`follow_symlinks`); inventory stores link targets. GNU `-h` semantics may differ. |
...
| `-P`     | `--absolute-names`                                  | FEATURE    | don't strip leading `/`s from file names                    | Rel-paths from input root today. (Store original -i directory in the meta table                                         |
...
|          | `--full-time`                                     | FEATURE      | print file time to its full resolution                                              | --list only relevant                                                |
...
|          | `--show-omitted-dirs`                             | FEATURE      | when listing or extracting, list each directory that does not match search criteria | (SELECT * FROM files WHERE exclusion_id IS NOT NULL)                |
